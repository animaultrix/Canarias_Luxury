...
class ItemOcio extends StatefulWidget {
  ...
  @override
  State<ItemOcio> createState() => _ItemState();
}


class _ItemState extends State<ItemOcio> {
  int quantity = 1;
              ...
              Container(
                child: Column(
                  ...
                  children: [
                    ...
                    Texto(
                      texto: "${widget.carruselImages.precio} € (${S.current.tansporteIncluido})", 
                      tamano: 18
                    ),
                    SizedBox(height: 18,),
                    Center(
                      child: Texto(
                        texto: S.current.numeroPersonas, 
                        tamano: 18,
                      ),
                    ),
                    Counter(
                    ...
                    Boton(texto: S.current.pedir, item: widget.carruselImages, quantity: quantity ,disableAddButton: widget.disableAddButton,),
                  ]
                ),
              )
            ]
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: ElevatedButton(
        child:  Text(texto),
        onPressed: disableAddButton ? null :() {
          Provider.of<OcioCartModel>(context, listen: false).addItem(item, quantity);
          // Elimina todas las pantallas de la pila de navegación y agrega Menu y Tienda
          Navigator.pushAndRemoveUntil(
            context,
            MaterialPageRoute(
              builder: (BuildContext context) => const Ocio(),
            ),
